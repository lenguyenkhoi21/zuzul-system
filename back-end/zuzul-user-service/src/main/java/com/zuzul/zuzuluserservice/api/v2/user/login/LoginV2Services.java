...
public class LoginV2Services {
    private final Keycloak keycloak;
    private final AdminClient adminClient;
    private final Logger logger = LoggerFactory.getLogger(LoginV2Services.class);
    private final UserInfoRepository userInfoRepository;

    public LoginV2POSTResponse login(POSTUserPayload payload) {
        ...
        if (token != null && !Objects.requireNonNull(compositeRoles).isEmpty()) {
            ...
            if (isUpdatedUser != null) {
                return LoginV2POSTResponse
                        .builder()
                        .userID(userInfo.getId())
                        .access_token(token.getAccess_token())
                        .role("USER")
                        .name(isUpdatedUser.getUserName())
